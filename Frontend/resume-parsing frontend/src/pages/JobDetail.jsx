...
import { getJob, getJobMatches, getCandidates, runMatch } from '../api/client';
...
const JobDetail = () => {
  ...
  const handleMatch = async () => {
    ...
    setMatching(true);
    try {
      const result = await runMatch(selectedCandidate, id);
  ...
  if (!job) return <div className="p-8 text-zinc-400">Job not found.</div>;
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto">
      ...
      <div className="bg-zinc-900 border border-zinc-800 rounded-xl p-6 mb-6">
        ...
        <p className="text-zinc-400 text-sm mb-4">{job.department}</p>

        <p className="text-xs text-zinc-500 mb-2 uppercase tracking-wide">Description</p>
        <div className="bg-zinc-800 rounded-lg p-4 max-h-48 overflow-y-auto text-zinc-300 text-sm whitespace-pre-wrap leading-relaxed mb-4">
          {job.description || 'No description available.'}
        </div>

        {job.requirements && (
          <>
            <p className="text-xs text-zinc-500 mb-2 uppercase tracking-wide">Requirements</p>
            <div className="bg-zinc-800 rounded-lg p-4 max-h-32 overflow-y-auto text-zinc-300 text-sm whitespace-pre-wrap leading-relaxed">
              {job.requirements}
            </div>
          </>
        )}
            ...
            <div key={m.id} className="bg-zinc-900 border border-zinc-800 rounded-xl p-5">
              <div className="flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-white font-medium">{m.candidateName}</p>
                  <button
